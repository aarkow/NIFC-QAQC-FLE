...
Usage: This tool should be used when engagement attributes need to be attributed to QAQCed firelines, or when the user requires a buffered fireline output without overlapping features, suitible for overlay analysis.
...
The tool will first identify if the user has input a complex (mutiple fire perimeters) and attribute them all based on the largest perimeter's attributes.
It will then attribute fireline engagement based on the user input fireline engagment buffer area.
"""
#import libraries
...
try:
    ...
    fc_Delete = ["FirePerims_RingBuffer","FirePerims_heldring","FirePerimsHeldRing","OpsData_HeldClip","OpsData_NotEngaged","OpsData_NotHeldClip"]
    for fc in fc_Delete:
        fc_path = os.path.join(localoutputws, fc)
        if arcpy.Exists(fc_path):
            arcpy.Delete_management(fc_path)
...
try:
    ...
    arcpy.analysis.PairwiseDissolve(QAQCed_Firelines,"Firelines_Diss","FeatureCategory",None,"MULTI_PART","")
    ...
    arcpy.analysis.PairwiseBuffer("Firelines_Diss","Firelines_DissBuff","50 Meters","NONE",None,"GEODESIC","0 DecimalDegrees")
    ...
    arcpy.analysis.PairwiseDissolve("Line_DissBuffInt","Line_DissBuffInt_diss","FeatureCategory;LineTypeValue",None,"MULTI_PART","")
    ...
    arcpy.management.JoinField("Line_CntOvp_Tbl","ORIG_OID","Line_DissBuffIntUnion","OBJECTID","LineTypeValue;FeatureCategory","NOT_USE_FM",None)
    ...
    arcpy.management.DeleteField("Line_Merge","CONCATENATE_FeatureCategory;CONCATENATE_LineTypeValue;FeatureCategory;LineTypeValue","DELETE_FIELDS")
    # Dissolve the final merged feature class by the count, maximum, and sum of line type values, and the text fields for line types and codes.
    arcpy.analysis.PairwiseDissolve("Line_Merge","IncName_LineBufferOvlpTreatments","COUNT_;MAX_LineTypeValue;SUM_LineTypeValue;LineType;LineTypeCodes",None,"MULTI_PART","")
    # Prepare the final layers for output by calculating fields where necessary.
    arcpy.management.MakeFeatureLayer("IncName_LineBufferOvlpTreatments","FnlLyr","MAX_LineTypeValue IS NULL")
    ...
    arcpy.management.MakeFeatureLayer("IncName_LineBufferOvlpTreatments","FnlLyr2","COUNT_ IS NULL")
    arcpy.management.CalculateField("FnlLyr2","COUNT_","1","PYTHON3","","TEXT","NO_ENFORCE_DOMAINS")
    fc_Delete = ["Line_Merge","Line_DissBuffErase_snglpt","Line_CntOvlp_snglpt","Firelines_Diss","Firelines_DissBuff_erase","Line_DissBuffIntUn_CntOvp","Line_DissBuffIntUnion","Line_DissBuffInt_diss","Line_DissBuffInt","Firelines_DissBuff","Line_CntOvp_Tbl","Line_CntOvp_Stats_MxVlu","Line_CntOvp_Stats_Sum","Line_CntOvp_Stats_Concat","Line_CntOvp_Stats_ValueConcat"]
    for fc in fc_Delete:
        fc_path = os.path.join(localoutputws, fc)
        if arcpy.Exists(fc_path):
            arcpy.Delete_management(fc_path) 
    ...
    print("Error")
    fc_Delete = ["Perims","Line_Merge","Line_DissBuffErase_snglpt","Line_CntOvlp_snglpt","Firelines_Diss","Firelines_DissBuff_erase","Line_DissBuffIntUn_CntOvp","Line_DissBuffIntUnion","Line_DissBuffInt_diss","Line_DissBuffInt","Firelines_DissBuff","Line_CntOvp_Tbl","Line_CntOvp_Stats_MxVlu","Line_CntOvp_Stats_Sum","Line_CntOvp_Stats_Concat","Line_CntOvp_Stats_ValueConcat"]
    for fc in fc_Delete:
        fc_path = os.path.join(localoutputws, fc)
        if arcpy.Exists(fc_path):
            arcpy.Delete_management(fc_path) 
...
try:
    arcpy.AddMessage("Creating Buffered Firelines Suitible for Overlay Analysis")
    print("Creating Buffered Firelines Suitible for Overlay Analysis")
    # Generate multiple buffers (rings) around the fire perimeter to identify different engagement zones.
    arcpy.analysis.MultipleRingBuffer("Perims","PerimMltRngBuff",FirelineEngagmentBuffList,"Meters","Buff","ALL","FULL","GEODESIC")
    # Isolate the 'held' area by creating a feature layer from the buffer based on the user-defined engagement distance.
    arcpy.management.MakeFeatureLayer("PerimMltRngBuff","PerimMltRngBuff_Held","Buff = "+(str(FirelineEngagmentBuffer)),None)
    # Isolate the 'burned over' area by creating a feature layer from the buffer where the engagement distance is negative.
    arcpy.management.MakeFeatureLayer("PerimMltRngBuff","PerimMltRngBuff_BurnedOver","Buff = -"+(str(FirelineEngagmentBuffer)),None)
    # Clip the fireline treatments to the 'held' area to identify lines that are effectively holding back the fire.
    arcpy.analysis.PairwiseClip("IncName_LineBufferOvlpTreatments","PerimMltRngBuff_Held","HeldLine",None)
    # Clip the fireline treatments to the 'burned over' area to identify lines that have been overrun by the fire.
    arcpy.analysis.PairwiseClip("IncName_LineBufferOvlpTreatments","PerimMltRngBuff_BurnedOver","BurnedOverLine",None)
    # Erase both the 'held' and 'burned over' areas from the overall treatments to identify lines that are not engaged.
    arcpy.analysis.PairwiseErase("IncName_LineBufferOvlpTreatments","PerimMltRngBuff","NotEngaged",None)
    # Check for each engagement type if there are any features present.
    # If none are found for a particular type, inform the user.
    #Held Line
    ...
    # Merge all engagement types into a single feature class for further analysis.
    arcpy.management.CalculateField("BurnedOverLine","Engagement",'"Burned Over"',"PYTHON3","","TEXT","NO_ENFORCE_DOMAINS")
    arcpy.management.Merge("HeldLine;BurnedOverLine;NotEngaged",IncidentName_BuffEnggmntLines_Datetime)
    # Add a field to store the geodesic area in square meters for the merged feature class.
    arcpy.management.AddField(IncidentName_BuffEnggmntLines_Datetime,"GeodesAreaMtrs","DOUBLE",None,None,None,"","NULLABLE","NON_REQUIRED","")
    arcpy.management.CalculateGeometryAttributes(IncidentName_BuffEnggmntLines_Datetime,"GeodesAreaMtrs AREA_GEODESIC","","SQUARE_METERS",None,"SAME_AS_INPUT")

    fc_Delete = ["Perims","IncName_LineBufferOvlpTreatments","PerimMltRngBuff","PerimMltRngBuff_Held","PerimMltRngBuff_BurnedOver","HeldLine","BurnedOverLine","NotEngaged"]
    for fc in fc_Delete:
        fc_path = os.path.join(localoutputws, fc)
        if arcpy.Exists(fc_path):
            arcpy.Delete_management(fc_path) 
