...
This has been commented out as github does not suport geodatabases so the US states feature class was not included.
...
The most important step in this tool in relateion to the fireline egagement metrics workflow is the duplicate shape analysis which can identify perimeters that have different attribution for the same shape and location.
"""
...
try:
    ...
    if countIrwinID == 0:
        print("No Duplicate Features with the same IRWIN ID Found. No Output Will be Created.")
        arcpy.AddMessage("No Duplicate Features with the same IRWIN ID Found. No Output Will be Created.")
        arcpy.management.Delete(IRWIN_ID_Field_Dups)
    else:
        arcpy.AddMessage("Features That Share the same IRWIN ID have been found. A subset will be placed in the Output.gdb and is Named: "+IRWIN_ID_Field_Dups)
        print("Features That Share the same IRWIN ID have been found. A subset will be placed in the Output.gdb and is Named: "+IRWIN_ID_Field_Dups)    
except:
    report_error()
